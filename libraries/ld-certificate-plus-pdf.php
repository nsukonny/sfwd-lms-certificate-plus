<?php
/**
 * Change default post2pdf_conv_post_to_pdf method for our
 *
 * @since 1.0.0
 * @deprecated 1.0.2 Use learndash_certificate_post_shortcode()
 */

function ld_init_pdf_convertor() {
	...
	if ( ! function_exists( 'post2pdf_conv_post_to_pdf' ) ) {
		...
		function post2pdf_conv_post_to_pdf() {
			...
			if ( class_exists( 'ACF' ) ) {
				$certificate_page_1 = get_field( 'certificate_page_1', $post_id );
				$certificate_page_2 = get_field( 'certificate_page_2', $post_id );
				$certificate_page_3 = get_field( 'certificate_page_3', $post_id );
				$html_content       = explode( '<!-- next page -->', $formatted_post );

				if ( ! empty( $certificate_page_1 ) ) {
					$pdf->Image( $certificate_page_1, '0', '0', $pageW, $pageH );
					$pdf->writeHTML( isset( $html_content[0] ) ? $html_content[0] : '', true, false, true, false, '' );
				}
				if ( ! empty( $certificate_page_2 ) ) {
					$pdf->AddPage();
					$pdf->Image( $certificate_page_2, '0', '0', $pageW, $pageH );
					$pdf->writeHTML( isset( $html_content[1] ) ? $html_content[1] : '', true, false, true, false, '' );
				}
				if ( ! empty( $certificate_page_3 ) ) {
					$pdf->AddPage();
					$pdf->Image( $certificate_page_3, '0', '0', $pageW, $pageH );
					$pdf->writeHTML( isset( $html_content[2] ) ? $html_content[2] : '', true, false, true, false, '' );
				}
			}
			...
			if ( ( ! isset( $certificate_page_1 ) || empty( $certificate_page_1 ) )
				...
				$img_file = learndash_get_thumb_path( $post_id );
...
add_action( 'template_redirect', 'ld_init_pdf_convertor', 1 );